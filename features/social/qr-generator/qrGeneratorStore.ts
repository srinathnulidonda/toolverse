// features/social/qr-generator/store.ts
    ...
    const handleRestore = useCallback((item: HistoryItem) => {
        ...
        switch (item.type) {
            ...
            case "email": {
                ...
                const subject = emailParts.length > 1 ? emailParts[1].split("&").find(p => p.startsWith("subject="))?.split("=")[1] || "" : "";
                const body = emailParts.length > 1 ? emailParts[1].split("&").find(p => p.startsWith("body="))?.split("=")[1] || "" : "";
