//features/social/qr-generator/ts/encode.ts
...
export function buildWifiString(d: WifiData): string {
  ...
  const pass = d.encryption === "nopass" ? "" : `P:${escapeWifi(d.password)};`;
...
export function buildLocationString(d: LocationData): string {
  ...
  return label ? `geo:${d.lat},${d.lng}?q=${encodeURIComponent(label)}` : `geo:${d.lat},${d.lng}`;
}
