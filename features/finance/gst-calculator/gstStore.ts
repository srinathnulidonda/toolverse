...
function validateHistory(raw: HistoryStorage | null, maxItems: number): GSTHistoryEntry[] {
    ...
    if (invalid.length > 0 && typeof window !== 'undefined' && process.env.NODE_ENV !== 'production') {
        console.warn(`Filtered out ${invalid.length} invalid GST history entries. They will be removed from storage.`);
    }
...
export function useGSTStore() {
    ...
    const history = useMemo(() => validateHistory(historyRaw, settings.maxHistoryItems), [historyRaw, settings.maxHistoryItems]);

    const saveToHistory = useCallback((entry: Omit<GSTHistoryEntry, "id" | "timestamp">) => {
    ...
    const updateEntry = useCallback((id: string, updates: Partial<GSTHistoryEntry>) => {
        setHistoryRaw((prev) => ({
            v: 1,
            data: (prev?.data ?? []).map(e =>
                e.id === id ? { ...e, ...updates } : e
    ...
    const searchHistory = useCallback((query: string): GSTHistoryEntry[] => {
    ...
    const getStatistics = useCallback(() => {
        ...
        const totalGSTCalculated = history.reduce((acc, e) => 
            acc + (e.calculation?.gstAmount || 0) + 
            (e.invoice?.totalGST || 0), 0
        );
        const averageGST = totalEntries > 0 ? totalGSTCalculated / totalEntries : 0;
        
        const typeUsage = history.reduce((acc, e) => {
            acc[e.type] = (acc[e.type] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const rateUsage = history.reduce((acc, e) => {
    ...
    const updateSettings = useCallback((updates: Partial<GSTSettings>) => {
        setSettingsRaw((prev) => ({
            v: 1,
    ...
    const exportHistory = useCallback((format: "json" | "csv") => {
        ...
        const headers = ["Date", "Title", "Type", "GST Amount", "Total Amount", "Customer", "Favorite"];
