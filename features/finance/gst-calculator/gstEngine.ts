...
export const HSN_CODES: HSNCode[] = [
    { code: "1001", description: "Wheat and meslin", gstRate: 0, cessRate: 0, category: "Agriculture" },
    { code: "0901", description: "Coffee", gstRate: 5, cessRate: 0, category: "Food & Beverages" },
    { code: "7113", description: "Jewellery - Gold", gstRate: 3, cessRate: 0, category: "Precious Metals" },
    { code: "8471", description: "Computers & peripherals", gstRate: 18, cessRate: 0, category: "Electronics" },
    { code: "8703", description: "Motor cars", gstRate: 28, cessRate: 15, category: "Automobiles" },
    { code: "9971", description: "Accounting services", gstRate: 18, cessRate: 0, category: "Services" },
    { code: "9973", description: "Legal services", gstRate: 18, cessRate: 0, category: "Services" },
    { code: "9996", description: "Hotel accommodation", gstRate: 12, cessRate: 0, category: "Hospitality" },
];
...
export function calculateBulkGST(
    ...
    const processedItems = items.map(item => {
        ...
        if (!item.quantity || item.quantity <= 0 || !item.pricePerUnit || item.pricePerUnit < 0) {
            console.warn(`Invalid bulk item: ${item.description}. Quantity and price must be positive.`);
        }
    ...
    const subtotal = processedItems.reduce((sum, item) => sum + (item.calculation?.originalAmount || 0), 0);
    const totalGST = processedItems.reduce((sum, item) => sum + (item.calculation?.gstAmount || 0), 0);
    const totalCess = processedItems.reduce((sum, item) => sum + (item.calculation?.cessAmount || 0), 0);
    const totalDiscount = processedItems.reduce((sum, item) => sum + (item.calculation?.discountAmount || 0), 0);
    const grandTotalBeforeRounding = processedItems.reduce((sum, item) => sum + (item.calculation?.finalAmount || 0), 0);
...
export function exportToCSV(items: BulkGSTItem[]): string {
    const headers = ["Description", "HSN/SAC", "Qty", "Rate", "Amount", "GST Rate", "GST Amount", "Cess", "Discount", "Total"];
...
export const SAMPLE_TEMPLATES = {
    ecommerce: {
        ...
        items: [
            { description: "Wireless Mouse", hsnCode: "8471", quantity: 2, pricePerUnit: 500, gstRate: 18, cessRate: 0, discountPercent: 10 },
            { description: "USB Cable", hsnCode: "8544", quantity: 3, pricePerUnit: 150, gstRate: 18, cessRate: 0, discountPercent: 5 },
            { description: "Laptop Bag", hsnCode: "4202", quantity: 1, pricePerUnit: 1200, gstRate: 18, cessRate: 0, discountPercent: 0 },
        ],
    },
    restaurant: {
        ...
        items: [
            { description: "Butter Chicken", hsnCode: "9963", quantity: 2, pricePerUnit: 350, gstRate: 5, cessRate: 0, discountPercent: 0 },
            { description: "Naan", hsnCode: "9963", quantity: 4, pricePerUnit: 40, gstRate: 5, cessRate: 0, discountPercent: 0 },
            { description: "Biryani", hsnCode: "9963", quantity: 1, pricePerUnit: 400, gstRate: 5, cessRate: 0, discountPercent: 0 },
            { description: "Cold Drink", hsnCode: "2202", quantity: 3, pricePerUnit: 60, gstRate: 12, cessRate: 0, discountPercent: 0 },
        ],
    },
    services: {
        ...
        items: [
            { description: "Web Design Services", hsnCode: "998314", quantity: 1, pricePerUnit: 25000, gstRate: 18, cessRate: 0, discountPercent: 0 },
            { description: "SEO Consultation", hsnCode: "998314", quantity: 3, pricePerUnit: 5000, gstRate: 18, cessRate: 0, discountPercent: 10 },
            { description: "Hosting (Annual)", hsnCode: "998314", quantity: 1, pricePerUnit: 3000, gstRate: 18, cessRate: 0, discountPercent: 0 },
        ],
    },
    automobile: {
        ...
        items: [
            { description: "Compact SUV", hsnCode: "8703", quantity: 1, pricePerUnit: 800000, gstRate: 28, cessRate: 15, discountPercent: 2 },
            { description: "Extended Warranty", hsnCode: "9971", quantity: 1, pricePerUnit: 15000, gstRate: 18, cessRate: 0, discountPercent: 0 },
        ],
    },
};