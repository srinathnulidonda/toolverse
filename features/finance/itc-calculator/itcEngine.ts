...
export const BLOCKED_ITC_CATEGORIES: BlockedITCCategory[] = [
    {
        ...
        description: "ITC blocked on motor vehicles and other conveyances except for specific business use",
        icon: "ti-car",
        examples: ["Cars", "Two-wheelers", "Trucks for general business use"],
    },
    {
        title: "Food & Beverages",
        description: "ITC not available on food, beverages, outdoor catering, and accommodation services",
        icon: "ti-chef-hat",
    ...
    },
    {
        ...
        description: "ITC blocked on membership of a club, health and fitness centre, and similar services",
    ...
    },
    {
        ...
        description: "Rent-a-cab, life insurance and health insurance services (except for specific conditions)",
        icon: "ti-plane",
...
export function calculateBulkITC(items: Array<{
    purchaseAmount: number;
    gstRate: number;
    blockedAmount?: number;
    reversedAmount?: number;
...
export function estimateMonthlyITC(calculation: ITCCalculation, period: ITCOptions["period"]): ITCCalculation {
...
export function checkITCEligibility(
    ...
    const blocked = BLOCKED_ITC_CATEGORIES.find(cat => 
...
export function exportITCToCSV(records: Array<{
    date: string;
    supplier: string;
    invoiceNo: string;
    amount: number;
