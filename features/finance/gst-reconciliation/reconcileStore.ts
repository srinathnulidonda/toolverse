...
export function useReconcileStore() {
  ...
  const getStatistics = () => {
    ...
    const totalSalesVariance = history.reduce(
      (acc, e) => acc + Math.abs(e.result.salesVariance),
      0
    );
    const totalPurchaseVariance = history.reduce(
      (acc, e) => acc + Math.abs(e.result.purchaseVariance),
      0
    );
    const totalITCVariance = history.reduce((acc, e) => acc + Math.abs(e.result.itcVariance), 0);

    const perfectMatches = history.filter(
      (e) =>
    ...
    return {
      ...
      perfectMatches,
      perfectMatchRate:
        totalEntries > 0 ? parseFloat(((perfectMatches / totalEntries) * 100).toFixed(1)) : 0,
      excessITCCases,
      unresolvedCount,
      riskDistribution,
    };
  ...
  const getYearlyComparison = (
    ...
    const months = yearEntries.map((e) => ({
      ...
      score: e.result.complianceScore,
      variance:
        Math.abs(e.result.salesVariance) +
        Math.abs(e.result.purchaseVariance) +
        Math.abs(e.result.itcVariance),
    }));

    const totalVariance = months.reduce((sum, m) => sum + m.variance, 0);
    const averageScore =
      months.length > 0 ? months.reduce((sum, m) => sum + m.score, 0) / months.length : 0;
