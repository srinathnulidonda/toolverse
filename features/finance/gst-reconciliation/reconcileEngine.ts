...
function generateRecommendations(
  ...
  if (Math.abs(purchaseVariance) > 0) {
    if (purchaseVariance > 0) {
      recommendations.push({
        ...
        action:
          "Follow up with suppliers to ensure timely GSTR-1 filing. Your ITC claim may be at risk.",
      });
    } else {
      recommendations.push({
        severity: "warning",
  ...
  if (Math.abs(itcVariance) > 0) {
    if (itcVariance > 0) {
      recommendations.push({
        ...
        action:
          "Reverse excess ITC claim immediately in next GSTR-3B filing to avoid interest and penalty.",
      });
    } else {
      recommendations.push({
        severity: "info",
  ...
  if (salesVariance === 0 && purchaseVariance === 0 && itcVariance === 0) {
    recommendations.push({
      ...
      description:
        "All your figures match perfectly between books and GST returns. Great job maintaining compliance!",
    });
  }
...
export function batchReconcile(periodsData: ReconciliationData[]): {
  ...
  const periodsWithIssues = results.filter(
    (r) =>
      Math.abs(r.salesVariance) > 0 ||
      Math.abs(r.purchaseVariance) > 0 ||
      Math.abs(r.itcVariance) > 0
  ).length;
...
export function parseCSVForReconciliation(csvContent: string): {
  ...
  const partyIdx = headers.findIndex(
    (h) => h.includes("party") || h.includes("name") || h.includes("supplier")
  );
...
export function matchInvoices(
  ...
  booksData.forEach((bookEntry) => {
    const gstrIndex = gstrData.findIndex(
      (gstrEntry, idx) =>
        !gstrMatched.has(idx) &&
