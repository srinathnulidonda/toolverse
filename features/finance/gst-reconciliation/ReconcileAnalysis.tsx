...
import { formatCurrency, estimatePenalty, getReconciliationStatus, MISMATCH_REASONS } from "./reconcileEngine";
...
export default function ReconcileAnalysis({ result, period }: ReconcileAnalysisProps) {
    ...
    const totalVarianceAmount = useMemo(() => {
        return Math.abs(result.salesVariance) + Math.abs(result.purchaseVariance) + Math.abs(result.itcVariance);
    }, [result]);
    ...
    const varianceBreakdown = useMemo(() => [
        { 
            ...
            color: "#10b981",
            percentage: totalVarianceAmount > 0 ? (Math.abs(result.salesVariance) / totalVarianceAmount) * 100 : 0
        },
        { 
            label: "Purchase Variance", 
            value: Math.abs(result.purchaseVariance), 
            color: "#3b82f6",
            percentage: totalVarianceAmount > 0 ? (Math.abs(result.purchaseVariance) / totalVarianceAmount) * 100 : 0
        },
        { 
            label: "ITC Variance", 
            value: Math.abs(result.itcVariance), 
            color: "#f59e0b",
            percentage: totalVarianceAmount > 0 ? (Math.abs(result.itcVariance) / totalVarianceAmount) * 100 : 0
        },
        ...
        <>
            <div className="ra-root">
                ...
                <div className="ra-status-banner" style={{ borderColor: status.color }}>
                    <div className="ra-status-icon" style={{ background: status.color + "20", color: status.color }}>
                        <i className={`ti ${status.status === "matched" ? "ti-check" : status.status === "minor_mismatch" ? "ti-alert-triangle" : "ti-alert-circle"}`} />
                    </div>
                            ...
                            {riskFactors.map((factor, idx) => (
                                <div key={idx} className={`ra-risk-factor ra-risk-factor--${factor.level}`}>
                                    <div className="ra-risk-icon">
                                        <i className={`ti ${
                                            factor.level === "high" ? "ti-alert-triangle" :
                                            factor.level === "medium" ? "ti-alert-circle" :
                                            "ti-check-circle"
                                        }`} />
                                    </div>
                        ...
                        <div className="ra-penalty-warning">
                            ...
                            <div className="ra-penalty-content">
                                <p className="ra-penalty-text">
                                    Based on excess ITC claim of {formatCurrency(result.itcVariance)}, here's the estimated financial impact if not corrected within 30 days:
                                </p>
                                <div className="ra-penalty-breakdown">
                                    ...
                                    <div className="ra-penalty-item">
                                        ...
                                        <span className="ra-penalty-value">{formatCurrency(penaltyEstimate.interestAmount)}</span>
                                    </div>
                                    <div className="ra-penalty-item">
                                        <span className="ra-penalty-label">Potential Penalty</span>
                                        <span className="ra-penalty-value">{formatCurrency(penaltyEstimate.penaltyAmount)}</span>
                                    </div>
                                    <div className="ra-penalty-item ra-penalty-item--total">
                                        <span className="ra-penalty-label"><strong>Total Liability</strong></span>
                                        <span className="ra-penalty-value"><strong>{formatCurrency(penaltyEstimate.totalLiability)}</strong></span>
                                    </div>
                                </div>
                                <p className="ra-penalty-action">
                                    <strong>Action Required:</strong> Reverse the excess ITC in your next GSTR-3B filing to avoid these charges.
                                </p>
                            </div>
                        </div>
                    </div>
                                ...
                                <div key={idx} className="ra-trend-bar-wrap">
                                    <div className="ra-trend-bar-container">
                                        <div 
                                            ...
                                            style={{ 
                                                height: `${point.score}%`,
                                                background: point.score >= 90 ? '#10b981' : point.score >= 70 ? '#f59e0b' : '#dc2626'
                                            }}
                                        />
                                    </div>
                    ...
                    <div className="ra-action-plan">
                        <div className="ra-action-step">
                            ...
                            <div className="ra-action-content">
                                ...
                                <div className="ra-action-desc">
                                    Cross-check each variance against individual invoices to identify specific mismatches.
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="ra-action-step">
                            ...
                            <div className="ra-action-content">
                                ...
                                <div className="ra-action-desc">
                                    For purchase mismatches, reach out to suppliers who haven't filed their GSTR-1 returns.
                                </div>
                            </div>
                        </div>
                        ...
                        {result.itcVariance > 0 && (
                            <div className="ra-action-step ra-action-step--urgent">
                                ...
                                <div className="ra-action-content">
                                    ...
                                    <div className="ra-action-desc">
                                        Immediately reverse the excess ITC claim of {formatCurrency(result.itcVariance)} to minimize interest and penalty exposure.
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            <style jsx>{`
                ...
                @media (prefers-color-scheme: dark) {
                    .ra-risk-factor--high { background: #1f1517; border-color: #3c1518; }
                    .ra-risk-factor--medium { background: #451a03; border-color: #78350f; }
                    .ra-risk-factor--low { background: #022c22; border-color: #064e3b; }
                }
                ...
                @media (prefers-color-scheme: dark) {
                    .ra-risk-factor--high .ra-risk-icon { background: #3c1518; color: #f87171; }
                    .ra-risk-factor--medium .ra-risk-icon { background: #78350f; color: #fbbf24; }
                    .ra-risk-factor--low .ra-risk-icon { background: #064e3b; color: #4ade80; }
                }
