...
export default function JSONValidatorWorkspace({ tool }: { tool: Tool }) {
    ...
    const TAB_VIEWS = [
        { id: "validate"   as const, label: "Validate", icon: "ti-circle-check",   description: "Validate JSON syntax" },
        { id: "schema"     as const, label: "Schema",   icon: "ti-shield-check",   description: "Schema validation" },
        { id: "compare"    as const, label: "Compare",  icon: "ti-git-compare",    description: "Compare documents" },
        ...
        <>
            <div className="jvw-root" ref={rootRef}>
                ...
                <div className="jvw-tab-content">
                    ...
                    {tabView === "validate" && (
                        <div className="jvw-validate-view">
                                ...
                                <div className={`jvw-status jvw-status--${result.valid ? "valid" : "invalid"}`}>
                                    <div className="jvw-status-left">
                                        ...
                                        {result.errors.length > 0 && (
                                            <span className="jvw-status-count">{result.errors.length} error{result.errors.length !== 1 ? 's' : ''}</span>
                                        )}
                                    </div>
                                    {result.valid && (
                            ...
                            <div className="jvw-body">
                                {/* Input Panel */}
                                <div className={`jvw-panel${mobilePanel === "input" ? " mobile-visible" : " mobile-hidden"}`}>
                                ...
                                {/* Output Panel */}
                                <div className={`jvw-panel${mobilePanel === "output" ? " mobile-visible" : " mobile-hidden"}`}>
                                    {!result && !input && (
                                        <div className="jvw-empty">
                                            ...
                                            <p className="jvw-empty-description">
                                                Paste JSON to check for syntax errors, structural issues, and security concerns
                                            </p>
                    ...
                    {/* Schema Tab */}
                    {tabView === "schema" && (
                        <SchemaValidator jsonInput={input} options={options} />
                    )}

                    {/* Compare Tab */}
                    {tabView === "compare" && (
                        <CompareMode leftDefault={input} />
                    )}
                        ...
                        <div className="jvw-history-view">
                            {history.length === 0 ? (
                                <div className="jvw-tab-empty">
                                    <div className="jvw-tab-empty-icon"><i className="ti ti-history" /></div>
                                            ...
                                            <div key={entry.id} className="jvw-history-item">
                                                <div className="jvw-history-item-header">
                                                    ...
                                                    <div className="jvw-history-item-meta">
                                                        <span className={`jvw-history-badge ${entry.result.valid ? "valid" : "invalid"}`}>
                                                            <i className={`ti ${entry.result.valid ? "ti-check" : "ti-x"}`} />
                                                            {entry.result.valid ? "Valid" : "Invalid"}
                                                        </span>
                                                        <span className="jvw-history-stat">{formatBytes(entry.result.stats.size)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
            ...
            <style jsx>{`
                ...
                .jvw-chrome-btn i { font-size: 13px; }
                ...
                .jvw-pill:hover { background: var(--bg-surface); color: var(--text); }
                .jvw-pill.active { background: var(--brand-light); color: var(--brand-text); }
                ...
                .jvw-setting-checkbox input { cursor: pointer; }
                .jvw-setting-checkbox span { font-size: 12px; color: var(--text-secondary); }
                ...
                .jvw-tab:hover { color: var(--text); }
                .jvw-tab.active { color: var(--text); }
                ...
                .jvw-action-btn i { font-size: 13px; }
                ...
                .jvw-switcher-tab.active { color: var(--text); }
                ...
                .jvw-panel-title i { font-size: 13px; }
                ...
                .jvw-textarea::placeholder { color: var(--text-disabled); }
                ...
                .jvw-view-result-btn:hover { background: var(--brand-hover); }
                .jvw-view-result-btn i { font-size: 16px; }
                ...
                .jvw-gutter-line { flex: 1; width: 0.5px; background: var(--border); }
                ...
                .jvw-empty-title { font-size: 16px; font-weight: 600; color: var(--text); margin: 0; }
                .jvw-empty-description { font-size: 13px; color: var(--text-tertiary); margin: 0; max-width: 400px; line-height: 1.6; }

                .jvw-empty-samples { display: flex; gap: 8px; flex-wrap: wrap; justify-content: center; }
                ...
                .jvw-empty-sample-btn:hover { background: var(--brand-light); color: var(--brand-text); border-color: var(--brand-border); }
                ...
                .jvw-tab-empty-title { font-size: 18px; font-weight: 600; color: var(--text); margin: 0; }
                .jvw-tab-empty-description { font-size: 14px; color: var(--text-tertiary); margin: 0; max-width: 400px; line-height: 1.6; }
                ...
                .jvw-history-title i { font-size: 15px; color: var(--text-secondary); }
                ...
                .jvw-history-item:hover { border-color: var(--brand-border); box-shadow: 0 2px 8px rgba(0,0,0,0.04); }

                .jvw-history-item-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; }

                .jvw-history-item-info { display: flex; flex-direction: column; gap: 4px; flex: 1; min-width: 0; }

                .jvw-history-item-title { font-size: 13px; font-weight: 600; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
                .jvw-history-item-time { font-size: 11px; color: var(--text-tertiary); font-family: var(--font-mono); }

                .jvw-history-item-meta { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
                ...
                .jvw-history-stat { font-size: 11px; font-weight: 500; font-family: var(--font-mono); color: var(--text-tertiary); }
                ...
                .jvw-footer-info { display: flex; align-items: center; gap: 6px; color: var(--text-tertiary); }
                .jvw-footer-info i { font-size: 13px; color: var(--brand); }
                .jvw-footer-stats { display: flex; align-items: center; gap: 8px; color: var(--text-disabled); font-family: var(--font-mono); flex-wrap: wrap; }
                ...
                @media (max-width: 768px) {
                    ...
                    .jvw-chrome-btn span,
                    .jvw-samples-label { display: none; }

                    .jvw-settings-row { flex-direction: column; align-items: stretch; }
                    .jvw-setting-group { width: 100%; }

                    .jvw-tab span { display: none; }
                    .jvw-tab { padding: 0 12px; }

                    .jvw-command-bar { flex-direction: column; align-items: stretch; }
                    .jvw-command-left,
                    .jvw-command-right { width: 100%; justify-content: space-between; }

                    .jvw-mobile-switcher { display: flex; }
                    ...
                    .jvw-mobile-cta { display: block; }

                    .jvw-footer { flex-direction: column; text-align: center; }
                }
                ...
                @media (prefers-reduced-motion: reduce) {
                    ...
                    .jvw-view-result-btn,
                    .jvw-empty-sample-btn,
                    .jvw-history-item { transition: none; }
                }
            `}</style>
        </>
    );
