// features/dev/slug-generator/utils.ts
export { formatBytes, downloadText } from '@/utils';

export type Separator = "-" | "_" | "." | "";
export type CaseStyle = "lowercase" | "uppercase" | "preserve" | "title" | "camel" | "pascal" | "snake";
...
export const SAMPLE_SLUGS = [
    ...
    { id: "technical", label: "Technical", text: "Understanding React's useEffect() Hook & Dependencies" },
...
export function generateSlug(text: string, options: SlugOptions): string {
    ...
    if (options.maxLength && slug.length > options.maxLength) {
        slug = options.smartTruncate && options.separator
