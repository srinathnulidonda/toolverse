...
export default function JSONWorkspace({ tool }: { tool: Tool }) {
    ...
    const handleDownload = useCallback((content: string, ext: string, mime: string) => {
    ...
    const TABS = [
        { id: "process"   as const, label: "Process",   icon: "ti-braces",      desc: "Minify or beautify JSON" },
        { id: "batch"     as const, label: "Batch",     icon: "ti-files",       desc: "Process multiple files"  },
        { id: "analysis"  as const, label: "Analysis",  icon: "ti-chart-bar",   desc: "JSON analysis & issues"  },
        { id: "transform" as const, label: "Transform", icon: "ti-transform",   desc: "Convert to CSV/YAML"     },
    ...
    return (
        <>
                ...
                <div className="jw-chrome">
                    <div className="jw-chrome-left">
                        <div className="jw-title">
                            <div className="jw-title-icon" style={{
                ...
                {showSettings && (
                    <div className="jw-settings">
                        <div className="jw-settings-grid">
                            {(options.mode === "beautify" || options.mode === "sort" || options.mode === "validate") && (
                                <div className="jw-setting-group">
                                    <label className="jw-setting-label">Indent Style</label>
                                    <select
                                        className="jw-select"
                                        value={options.indentStyle}
                                        onChange={e => setOptions(p => ({ ...p, indentStyle: e.target.value as IndentStyle }))}
                                    >
                            ...
                            <div className="jw-setting-group">
                                ...
                                <select
                                    ...
                                    onChange={e => setOptions(p => ({ ...p, sortOrder: e.target.value as "asc" | "desc" }))}
                                >
                            ...
                            {[
                                ...
                                <label key={key} className="jw-toggle">
                                    ...
                                    />
                                    <div className="jw-toggle-track"><div className="jw-toggle-thumb" /></div>
                                    <span>{label}</span>
                                </label>
                            ))}
                        </div>
                ...
                <div className="jw-content">
                    ...
                    {tabView === "process" && (
                        <div className="jw-process-view">
                            ...
                            <div className="jw-body">
                                ...
                                {/* Input */}
                                <div className={`jw-panel ${mobilePanel === "input" ? "mob-visible" : "mob-hidden"}`}>
                                        ...
                                        <div className="jw-panel-actions">
                                            {input && (
                                                <>
                                                    ...
                                                    {error
                                                        ? <span className="jw-error-badge"><i className="ti ti-alert-circle" /> Invalid</span>
                                                        : result && <span className="jw-valid-badge"><i className="ti ti-check" /> Valid</span>
                                                    }
                                                </>
                                            )}
                                            <button
                                    ...
                                    {input && result && (
                                        <div className="jw-mob-cta">
                                            <button type="button" className="jw-cta-btn" onClick={goToOutput}>
                                                <i className="ti ti-sparkles" />
                                                View {options.mode === "minify" ? "Minified" : options.mode === "beautify" ? "Beautified" : options.mode === "sort" ? "Sorted" : "Validated"} JSON
                                ...
                                <div className="jw-gutter">
                                    ...
                                    <div className="jw-gutter-node">
                                        <i className={`ti ${modeConfig[options.mode].icon}`} style={{ color: modeConfig[options.mode].color }} />
                                    </div>
                                    <div className="jw-gutter-line" />
                                </div>

                                {/* Output */}
                                <div className={`jw-panel ${mobilePanel === "output" ? "mob-visible" : "mob-hidden"}`}>
                                    <div className="jw-panel-bar">
                                        <div className="jw-panel-label">
                                            <i className={`ti ${modeConfig[options.mode].icon}`} />
                                            {options.mode === "minify"   ? "Minified" :
                                             options.mode === "beautify" ? "Beautified" :
                                             options.mode === "sort"     ? "Sorted" :
                                                                           "Validated"} JSON
                                        </div>
                                        <div className="jw-panel-actions">
                                            {result && (
                                    ...
                                    {!result && !error && (
                                        <div className="jw-empty">
                                            ...
                                            <div className="jw-empty-samples">
                                                {Object.entries(SAMPLE_TEMPLATES).slice(0, 2).map(([key, s]) => (
                                                    <button
                                                        key={key}
                                                        type="button"
                                        ...
                                        <>
                                            ...
                                            <div className="jw-stats-bar">
                                                <div className="jw-stat">
                                                    ...
                                                    <span className="jw-stat-value">{formatBytes(result.stats.original)}</span>
                                                </div>
                                                <div className="jw-stat">
                                                    <i className="ti ti-sparkles" />
                                                    <span className="jw-stat-label">Result</span>
                                                    <span className="jw-stat-value">{formatBytes(result.stats.processed)}</span>
                                                </div>
                                                {result.stats.savings !== 0 && (
                                                    <div className={`jw-stat jw-stat--highlight ${result.stats.savings > 0 ? "good" : "warn"}`}>
                                                        <i className={`ti ${result.stats.savings > 0 ? "ti-trending-down" : "ti-trending-up"}`} />
                                                        <span className="jw-stat-label">{result.stats.savings > 0 ? "Saved" : "Added"}</span>
                                                        <span className={`jw-stat-value ${result.stats.savings > 0 ? "good" : "warn"}`}>
                                                            {formatBytes(Math.abs(result.stats.savings))} ({Math.abs(result.stats.savingsPercent)}%)
                                                        </span>
                                                    </div>
                                                )}
                                                <div className="jw-stat">
                    ...
                    {tabView === "analysis" && (
                        result ? (
                            ...
                            <div className="jw-tab-empty">
                                <div className="jw-tab-empty-icon"><i className="ti ti-chart-bar" /></div>
                                <h3>JSON Analysis</h3>
                                <p>Add valid JSON to see structure analysis, type distribution, schema inference, and issue detection.</p>
                                <button type="button" className="jw-tab-empty-btn" onClick={() => setTabView("process")}>
                                    Go to Processor
                                </button>
                            </div>
                        )
                        ...
                        <div className="jw-transform-view">
                            ...
                            <div className="jw-transform-section">
                                ...
                                <div className="jw-convert-grid">
                                    ...
                                    >
                                        <div className="jw-convert-card-icon" style={{ background: "#eff6ff", color: "#2563eb" }}>
                                    ...
                                    >
                                        <div className="jw-convert-card-icon" style={{ background: "#faf5ff", color: "#7c3aed" }}>
                                    ...
                                    >
                                        <div className="jw-convert-card-icon" style={{ background: "#f0fdf4", color: "#16a34a" }}>
                                    ...
                                    >
                                        <div className="jw-convert-card-icon" style={{ background: "#eff6ff", color: "#0284c7" }}>
                    ...
                    {tabView === "history" && (
                        <div className="jw-history">
                            {history.length === 0 ? (
                                <div className="jw-tab-empty">
                                    <div className="jw-tab-empty-icon"><i className="ti ti-history" /></div>
                                            ...
                                            <div key={entry.id} className="jw-history-item">
                                                ...
                                                <div className="jw-history-item-right">
                                                    ...
                                                    {entry.result.stats.savingsPercent !== 0 && (
                                                        <span className={`jw-history-savings ${entry.result.stats.savings > 0 ? "good" : "warn"}`}>
                                                            {entry.result.stats.savings > 0 ? "-" : "+"}{Math.abs(entry.result.stats.savingsPercent)}%
                                                        </span>
                                                    )}
                                                    <button
                                                        type="button"
                                                        className="jw-icon-btn"
                                                        onClick={() => { setInput(entry.input); setTabView("process"); }}
                                                        title="Restore"
                                                    >
            ...
            <style jsx>{`
                ...
                .jw-chrome-left,
                .jw-chrome-right { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
                ...
                .jw-mode-pill i { font-size: 12px; }
                .jw-mode-pill:hover { color: var(--text); background: var(--bg-surface); }
                ...
                .jw-settings-btn:hover { background: var(--bg-surface); color: var(--text); }
                .jw-settings-btn.active { background: var(--brand-light); color: var(--brand-text); border-color: var(--brand-border); }
                .jw-settings-btn i { font-size: 13px; }
                ...
                .jw-toggle input:checked ~ .jw-toggle-track { background: var(--brand); }
                ...
                .jw-toggle input:checked ~ .jw-toggle-track .jw-toggle-thumb { transform: translateX(16px); }
                .jw-toggle span { font-size: 12px; font-weight: 500; color: var(--text-secondary); }
                ...
                .jw-tab i { font-size: 14px; }
                .jw-tab:hover { color: var(--text-secondary); }
                ...
                .jw-toolbar-left,
                .jw-toolbar-right { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
                ...
                .jw-export-chips { display: flex; align-items: center; gap: 4px; }
                ...
                .jw-sw-tab.active { color: var(--text); }
                ...
                .jw-panel-label i { font-size: 12px; }
                ...
                @media (prefers-color-scheme: dark) {
                    .jw-error-badge { background: #1f1517; color: #f87171; }
                }
                ...
                @media (prefers-color-scheme: dark) {
                    .jw-valid-badge { background: #022c22; color: #4ade80; }
                }
                ...
                .jw-icon-btn:disabled { opacity: 0.3; cursor: not-allowed; }
                ...
                .jw-copy-btn:hover { background: var(--bg-surface); color: var(--text); }
                .jw-copy-btn.copied { background: var(--brand-light); color: var(--brand-text); border-color: var(--brand-border); }
                .jw-copy-btn i { font-size: 11px; }
                ...
                .jw-textarea--error { border-left-color: #dc2626; }

                @media (prefers-color-scheme: dark) {
                    .jw-textarea--error { border-left-color: #f87171; }
                }

                .jw-textarea::placeholder { color: var(--text-disabled); }
                ...
                @media (prefers-color-scheme: dark) {
                    .jw-error-banner { background: #1f1517; color: #f87171; border-color: #3c1518; }
                }

                .jw-error-banner i { font-size: 14px; flex-shrink: 0; }
                ...
                .jw-cta-btn:hover { background: var(--brand-hover); }
                .jw-cta-btn i { font-size: 15px; }
                .jw-cta-btn i:last-child { margin-left: auto; opacity: 0.7; }
                ...
                .jw-gutter-line { flex: 1; width: 0.5px; background: var(--border); }
                ...
                @media (prefers-color-scheme: dark) {
                    .jw-empty-icon--error { background: #1f1517; border-color: #3c1518; color: #f87171; }
                }

                .jw-empty-title { font-size: 16px; font-weight: 700; color: var(--text); margin: 0; }
                .jw-empty-desc  { font-size: 13px; color: var(--text-tertiary); margin: 0; max-width: 300px; line-height: 1.6; }

                .jw-empty-samples { display: flex; gap: 8px; flex-wrap: wrap; justify-content: center; }
                ...
                .jw-stat i { font-size: 12px; color: var(--text-tertiary); }
                .jw-stat-label { color: var(--text-tertiary); font-weight: 500; }
                .jw-stat-value { color: var(--text); font-weight: 700; font-family: var(--font-mono); }

                .jw-stat-value.good { color: #16a34a; }
                .jw-stat-value.warn { color: #d97706; }

                @media (prefers-color-scheme: dark) {
                    .jw-stat-value.good { color: #4ade80; }
                    .jw-stat-value.warn { color: #fbbf24; }
                }
                ...
                .jw-stat--highlight.good { background: #dcfce7; }
                .jw-stat--highlight.warn { background: #fef3c7; }

                @media (prefers-color-scheme: dark) {
                    .jw-stat--highlight.good { background: #022c22; }
                    .jw-stat--highlight.warn { background: #451a03; }
                }
                ...
                .jw-download-btn:hover { background: var(--brand-hover); }
                .jw-download-btn i { font-size: 12px; }
                ...
                .jw-mob-btn:hover { background: var(--bg-surface); color: var(--text); }
                .jw-mob-btn.copied { background: var(--brand-light); color: var(--brand-text); border-color: var(--brand-border); }
                .jw-mob-btn i { font-size: 15px; }
                ...
                .jw-tab-empty h3 { font-size: 18px; font-weight: 700; color: var(--text); margin: 0; }
                .jw-tab-empty p  { font-size: 13px; color: var(--text-tertiary); margin: 0; max-width: 380px; line-height: 1.6; }
                ...
                .jw-tab-empty-btn:hover { background: var(--brand-hover); }
                ...
                .jw-transform-title i { font-size: 15px; color: var(--text-secondary); }
                ...
                .jw-transform-section-title i { font-size: 13px; }
                ...
                .jw-jsonpath-input:focus { border-color: var(--brand); }
                .jw-jsonpath-input::placeholder { color: var(--text-disabled); }
                ...
                .jw-jsonpath-btn:hover:not(:disabled) { background: var(--brand-hover); }
                .jw-jsonpath-btn:disabled { opacity: 0.4; cursor: not-allowed; }
                .jw-jsonpath-btn i { font-size: 13px; }
                ...
                .jw-convert-card:disabled { opacity: 0.4; cursor: not-allowed; }
                ...
                .jw-convert-card-action i { font-size: 12px; }
                ...
                .jw-transform-empty p { margin: 0; max-width: 320px; line-height: 1.6; }
                ...
                .jw-history-title i { font-size: 15px; color: var(--text-secondary); }
                ...
                .jw-ghost-btn:hover { background: var(--bg-card); color: var(--text); }
                .jw-ghost-btn i { font-size: 13px; }
                ...
                .jw-history-item:hover { border-color: var(--brand-border); }
                ...
                .jw-history-savings.good { color: #16a34a; }
                .jw-history-savings.warn { color: #d97706; }

                @media (prefers-color-scheme: dark) {
                    .jw-history-savings.good { color: #4ade80; }
                    .jw-history-savings.warn { color: #fbbf24; }
                }
                ...
                .jw-footer-left { display: flex; align-items: center; gap: 6px; color: var(--text-tertiary); }
                .jw-footer-left i { font-size: 13px; color: var(--brand); }
                ...
                .jw-valid   { color: #16a34a; display: flex; align-items: center; gap: 3px; }
                .jw-invalid { color: #dc2626; display: flex; align-items: center; gap: 3px; }

                @media (prefers-color-scheme: dark) {
                    .jw-valid   { color: #4ade80; }
                    .jw-invalid { color: #f87171; }
                }

                /* Mobile */
                @media (max-width: 768px) {
                    .jw-root { border-radius: 0; border-left: none; border-right: none; min-height: 100dvh; }

                    .jw-chrome { gap: 8px; }
                    .jw-settings-btn span { display: none; }

                    .jw-mode-pills { gap: 2px; }
                    .jw-mode-pill  { padding: 0 6px; }
                    .jw-mode-label { display: none; }

                    .jw-settings-grid { flex-direction: column; gap: 10px; }

                    .jw-tabs { padding: 0 8px; }
                    .jw-tab span { display: none; }
                    .jw-tab { padding: 0 10px; }

                    .jw-toolbar { flex-direction: column; align-items: stretch; }
                    .jw-toolbar-left { overflow-x: auto; flex-wrap: nowrap; }

                    .jw-mobile-switcher { display: flex; }
                    ...
                    .jw-panel.mob-visible { z-index: 1; visibility: visible; }
                    .jw-panel.mob-hidden  { z-index: 0; visibility: hidden; pointer-events: none; }

                    .jw-mob-cta     { display: block; }
                    .jw-mob-actions { display: flex; }
                    .jw-download-btn { display: none; }

                    .jw-stats-bar { flex-wrap: wrap; gap: 8px; }

                    .jw-convert-grid { grid-template-columns: 1fr 1fr; }

                    .jw-footer { flex-direction: column; text-align: center; gap: 6px; }
                    .jw-footer-right { justify-content: center; }
                }

                @media (max-width: 480px) {
                    .jw-convert-grid { grid-template-columns: 1fr; }
                }

                @media (prefers-reduced-motion: reduce) {
                    .jw-mode-pill, .jw-settings-btn, .jw-tab, .jw-sample-btn,
                    .jw-sw-tab, .jw-icon-btn, .jw-copy-btn, .jw-cta-btn,
                    .jw-mob-btn, .jw-download-btn, .jw-history-item,
                    .jw-convert-card, .jw-toggle-track, .jw-toggle-thumb,
                    .jw-title-icon, .jw-textarea { transition: none; }
                }
            `}</style>
        </>
    );
