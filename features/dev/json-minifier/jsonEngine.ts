// features/dev/json-minifier/jsonEngine.ts
...
export function processJSON(input: string, options: ProcessOptions = DEFAULT_OPTIONS): ProcessResult {
        ...
        return Object.fromEntries(
            Object.entries(obj)
                ...
                .filter(([, v]) => {
                    ...
                    if (type === "object" && v && typeof v === "object" && !Array.isArray(v) && Object.keys(v).length === 0) return false;
                    return true;
                })
        );
    }
    ...
    if (typeof data === "object") {
        return {
            ...
            children: Object.fromEntries(
                Object.entries(data).slice(0, 10).map(([k, v]) => [k, buildSchema(v, depth + 1)])
            )
        };
    }
...
export function analyzeIssues(data: any, raw: string): JSONIssue[] {
    ...
    function traverse(obj: any, path: string): void {
        ...
        if (Array.isArray(obj)) {
            if (obj.length === 0) {
                issues.push({ type: "info", message: `Empty array at "${path}"`, path, rule: "no-empty-array" });
            }
            // Check for mixed types in arrays
            if (obj.length > 1) {
                const types = new Set(obj.map((v: any) => Array.isArray(v) ? "array" : typeof v));
                if (types.size > 1) {
                    issues.push({ type: "warning", message: `Mixed types in array at "${path}"`, path, rule: "consistent-array-types" });
                }
            }
        ...
        if (typeof obj === "object") {
            const keys = Object.keys(obj);
            if (keys.length === 0) {
                issues.push({ type: "info", message: `Empty object at "${path}"`, path, rule: "no-empty-object" });
            }
            keys.forEach(k => {
                if (k !== k.trim()) {
                    issues.push({ type: "warning", message: `Key "${k}" has leading/trailing whitespace`, path: `${path}.${k}`, rule: "no-whitespace-keys" });
                }
        ...
        if (typeof obj === "string" && obj === "") {
            issues.push({ type: "info", message: `Empty string at "${path}"`, path, rule: "no-empty-string" });
        }

        if (typeof obj === "number" && !isFinite(obj)) {
            issues.push({ type: "error", message: `Non-finite number at "${path}"`, path, rule: "no-non-finite" });
        }
    }
    ...
    if (new Blob([raw]).size > 100 * 1024) {
        issues.push({ type: "warning", message: "File is larger than 100KB — consider splitting", rule: "max-size" });
    }
...
function calculateStats(original: string, processed: string, parsed: any): JSONStats {
    ...
    let keys = 0, arrays = 0, objects = 0, strings = 0, numbers = 0, booleans = 0, nulls = 0, depth = 0;
    ...
    function traverse(obj: any, d = 0): void {
        ...
        if (Array.isArray(obj))                     { arrays++;  obj.forEach(v => traverse(v, d + 1)); return; }
        if (obj && typeof obj === "object") {
            objects++;
