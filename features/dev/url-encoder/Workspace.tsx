...
export default function UrlEncoderWorkspace({ tool }: { tool: Tool }) {
    ...
    const output = useMemo(() => {
        ...
        try {
            return mode === "encode" 
                ...
                : decodeUrl(input, options).result;
        } catch {
            return "";
        }
    }, [mode, input, options]);
    ...
    const loadSample = useCallback((preset: keyof typeof SAMPLE_URLS) => {
                ...
                <div className="uew-content">
                    {viewTab === "single" && (
                        <>
                            ...
                            {outputTab === "breakdown" && (
                                <UrlBreakdown urlParts={parsedUrl} />
                            )}

                            {outputTab === "diff" && (
                                <UrlDiff input={input} output={output} />
                            )}
                            ...
                            {output && (
                                <div className="uew-stats-bar">
                                    ...
                                    <div className="uew-stat">
                                        ...
                                        <span className={`uew-stat-value${stats.delta > 0 ? " warn" : stats.delta < 0 ? " good" : ""}`}>
                    ...
                    {viewTab === "batch" && (
                        <UrlBatch mode={mode} options={options} />
                    )}

                    {viewTab === "compare" && (
                        <UrlCompare options={options} />
                    )}
