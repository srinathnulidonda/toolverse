...
export default function DiffCheckerWorkspace({ tool }: { tool: Tool }) {
    ...
    const { history, addToHistory, clearHistory, searchHistory } = useDiffStore();
    ...
    const handleExport = useCallback((format: "patch" | "html" | "json") => {
    ...
    }, [diffResult, originalText, modifiedText, originalFilename, modifiedFilename, options, fileType, addToHistory]);
            ...
            <div className="dc-root">
                    ...
                    <div className="dc-command-right">
                        ...
                        <div className="dc-view-toggle">
                            {(["split", "unified"] as DiffViewMode[]).map((mode) => (
                                ...
                                >
                                    <i className={`ti ${mode === "split" ? "ti-layout-columns" : "ti-layout-list"}`} />
                ...
                <div className="dc-content">
                    ...
                    <div className="dc-input-section">
                        <div className={`dc-input-panel${mobilePanel === "original" ? " mobile-visible" : " mobile-hidden"}`}>
                            <div className="dc-panel-header">
                                <div className="dc-panel-title">
                                    <i className="ti ti-file" />
                                    <span>Original</span>
                                    {originalFilename && (
                                        <span className="dc-filename">{originalFilename}</span>
                                    )}
                                </div>
                        ...
                        <div className={`dc-input-panel${mobilePanel === "modified" ? " mobile-visible" : " mobile-hidden"}`}>
                            <div className="dc-panel-header">
                                <div className="dc-panel-title">
                                    <i className="ti ti-file-diff" />
                                    <span>Modified</span>
                                    {modifiedFilename && (
                                        <span className="dc-filename">{modifiedFilename}</span>
                                    )}
                                </div>
                                <div className="dc-panel-actions">
                                    <input
                                                ...
                                                {history.slice(0, 20).map((entry) => (
                                                    <div key={entry.id} className="dc-history-item">
                                                        <div className="dc-history-item-header">
                                                            <span className="dc-history-item-title">
                                                                {entry.title}
                                                            </span>
