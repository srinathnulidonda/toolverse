...
function jsonToYaml(value: unknown, indent = 0): string {
    ...
    if (typeof value === "string") {
        if (/[:\n#&*?|<>{}\[\],]/.test(value) || value.includes("  ")) return `"${value.replace(/"/g, '\\"')}"`;
        return value;
    }
...
function jsonToToml(value: unknown): string {
    ...
    for (const [k, v] of complex) {
        if (Array.isArray(v)) {
            ...
            if (isSimple) {
                lines.push(`${k} = [${(v as unknown[]).map((i) => typeof i === "string" ? `"${i}"` : String(i)).join(", ")}]`);
            } else {
...
export default function JsonFormatterWorkspace({ tool }: { tool: Tool }) {
    ...
    const savings = inputBytes > 0 && outputBytes < inputBytes
        ? Math.round((1 - outputBytes / inputBytes) * 100) : 0;
    ...
    const handleDownload = useCallback((ext = "json", content = output) => {
        if (!content) return;
    ...
    const handleConvert = useCallback((target: ConvertTarget) => {
        ...
        <>
            <div className="jf-root" role="main" aria-label="JSON Formatter">
                    ...
                    >
                        ...
                        {input.trim() && (
                            <span className={`jf-mobile-badge ${hasError ? "error" : "valid"}`}>
                                {hasError ? <i className="ti ti-alert-circle" aria-hidden="true" /> : <i className="ti ti-check" aria-hidden="true" />}
                            </span>
                        )}
                    </button>
                    <button
                ...
                <div className="jf-body">
                    ...
                    {/* Input Panel */}
                    <div className={`jf-panel jf-panel-input${mobilePanel === "input" ? " mobile-visible" : " mobile-hidden"}`}>
                        <div className="jf-panel-header">
                            ...
                            <div className="jf-panel-meta">
                                {input.length > 0 && (
                                    <span className="jf-meta-text">{fmtSize(input)}</span>
                                )}
                                {input.trim() && (
                                    hasError ? (
                        ...
                        <div className="jf-textarea-wrap">
                            ...
                            {!input && (
                                <div className="jf-placeholder-hints" aria-hidden="true">
                                    <span><span className="jf-hint-kw">{"{"}</span>name: 'Alice'<span className="jf-hint-kw">{"}"}</span> ← unquoted keys</span>
                        ...
                        {hasError && (
                            <div className="jf-error-bar" id="jf-error-msg" role="alert">
                                ...
                                <div>
                                    ...
                                    <span className="jf-error-tip">Auto-fixes: unquoted keys, trailing commas, single quotes, key=value pairs. Check for mismatched brackets.</span>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {/* Output Panel */}
                    <div className={`jf-panel jf-panel-output${mobilePanel === "output" ? " mobile-visible" : " mobile-hidden"}`}>
                        <div className="jf-panel-header">
                            ...
                            {showMoreMenu && (
                                <div className="jf-mob-menu" role="menu">
                                    ...
                                    <button
                                        ...
                                        onClick={() => { setSortKeys((s) => !s); setShowMoreMenu(false); }}
                                    >
                                        <i className="ti ti-sort-ascending-letters" aria-hidden="true" />
                                        Sort keys {sortKeys ? "✓" : ""}
                                    </button>
