...
function analyzeJson(value: unknown, depth = 0): Omit<Stats, "bytes"> {
    const acc: Omit<Stats, "bytes"> = {
        keys: 0, depth, arrays: 0, objects: 0,
        strings: 0, numbers: 0, booleans: 0, nulls: 0, totalNodes: 1,
    };
    if (value === null) { acc.nulls = 1; return acc; }
    if (typeof value === "string") { acc.strings = 1; return acc; }
    if (typeof value === "number") { acc.numbers = 1; return acc; }
    if (typeof value === "boolean") { acc.booleans = 1; return acc; }
    if (Array.isArray(value)) {
        acc.arrays = 1;
        for (const v of value) {
            const c = analyzeJson(v, depth + 1);
            acc.keys += c.keys; acc.depth = Math.max(acc.depth, c.depth);
            acc.arrays += c.arrays; acc.objects += c.objects;
            acc.strings += c.strings; acc.numbers += c.numbers;
            acc.booleans += c.booleans; acc.nulls += c.nulls;
...
export default function JsonStats({ value, rawText }: StatsProps) {
    ...
    const structure: StatItem[] = [
        ...
        { label: "Total nodes", value: stats.totalNodes.toLocaleString(), icon: "ti-box", group: "structure" },
    ...
    const types: StatItem[] = [
        ...
        { label: "Booleans", value: stats.booleans.toLocaleString(), icon: "ti-toggle-left", group: "type" },
        { label: "Nulls", value: stats.nulls.toLocaleString(), icon: "ti-circle-dashed", group: "type" },
    ];
