...
const MODE_COLORS: Record<MinifyMode, string> = {
  minify: "#f59e0b",
  compress: "#3b82f6",
  mangle: "#8b5cf6",
};

const MODES: MinifyMode[] = ["minify", "compress", "mangle"];

const TABS = [
  { id: "minify" as const, label: "Minify", icon: "ti-file-zip" },
  { id: "batch" as const, label: "Batch", icon: "ti-files" },
  { id: "analysis" as const, label: "Analysis", icon: "ti-chart-bar" },
  { id: "history" as const, label: "History", icon: "ti-history" },
] as const;

export default function JSMinifierWorkspace({ tool: _tool }: { tool: Tool }) {
    ...
    <div className={styles.jwRoot} ref={rootRef}>
      <div className={styles.jwChrome}>
        ...
        <div className={styles.jwChromeRight}>
          <div className={styles.jwModePills}>
            {MODES.map((m) => (
              <button
                key={m}
                type="button"
                className={`${styles.jwModePill} ${options.mode === m ? styles.jwModePillActive : ""}`}
                style={
                  options.mode === m
                    ? {
          ...
          <button
            type="button"
            className={`${styles.jwSettingsBtn} ${showSettings ? styles.active : ""}`}
          ...
          <div className={styles.jwSettingsGrid}>
            {(
              [
                ...
                { key: "removeConsole", label: "Remove console.*" },
                { key: "removeDebugger", label: "Remove debugger" },
                { key: "collapseWhitespace", label: "Collapse Whitespace" },
                { key: "mangle", label: "Mangle Variables" },
                { key: "deadCodeElimination", label: "Dead Code Elimination" },
              ] as { key: keyof MinifyOptions; label: string }[]
            ).map(({ key, label }) => (
              <label key={key} className={styles.jwToggle}>
                <input
                  type="checkbox"
                  checked={options[key] as boolean}
                  onChange={(e) => toggleOption(key, e.target.checked as MinifyOptions[typeof key])}
                />
      ...
      <div className={styles.jwTabsBar}>
        <nav className={styles.jwTabs} role="tablist">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              type="button"
              ...
              className={`${styles.jwTab} ${tabView === tab.id ? styles.active : ""}`}
      ...
      <div className={styles.jwContent} role="tabpanel">
        {tabView === "minify" && (
          <div className={styles.jwMinifyView}>
            ...
            <div className={styles.jwMobileSwitcher}>
              <button
                type="button"
                className={`${styles.jwSwTab} ${mobilePanel === "input" ? styles.active : ""}`}
                onClick={goToInput}
              >
              ...
              <button
                type="button"
                className={`${styles.jwSwTab} ${mobilePanel === "output" ? styles.active : ""}`}
                onClick={goToOutput}
              >
                <i className="ti ti-file-zip" />
                Minified
                {result && mobilePanel !== "output" && (
                  <span className={styles.jwSwDot} />
                )}
              </button>
            </div>
            ...
            <div className={styles.jwBody}>
              <div
                className={`${styles.jwPanel} ${
                  mobilePanel === "input" ? styles.mobVisible : styles.mobHidden
                }`}
              >
                <div className={styles.jwPanelBar}>
                  ...
                  <div className={styles.jwPanelActions}>
                    {input && (
                      <>
                        <span className={styles.jwCharCount}>
              ...
              <div className={styles.jwGutter} aria-hidden="true">
                ...
                <div className={styles.jwPanelBar}>
                  ...
                  <div className={styles.jwPanelActions}>
                    {result && (
                      <>
                        <span className={styles.jwCharCount}>
                          {result.output.length.toLocaleString()} ch
                        </span>
                        <button
                          type="button"
                          className={`${styles.jwCopyBtn} ${
                            copiedKey === "out" ? styles.copied : ""
                          }`}
                        ...
                        >
                          <i
                            className={`ti ${
                              copiedKey === "out" ? "ti-check" : "ti-copy"
                            }`}
                          />
                ...
                {!result ? (
                  <div className={styles.jwEmpty}>
                    ...
                    <p className={styles.jwEmptyDesc}>
                      Paste code on the left, load a sample, or drop a file to
                      reduce your bundle size instantly.
                    </p>
                      ...
                      {Object.entries(SAMPLE_TEMPLATES)
                        ...
                        .map(([key, s]) => (
                          <button
                            ...
                            onClick={() =>
                              loadSample(key as keyof typeof SAMPLE_TEMPLATES)
                            }
                          >
                            Try {s.name}
                          </button>
                  ...
                  <>
                    ...
                    <div className={styles.jwStatsBar}>
                      ...
                      <div
                        className={`${styles.jwStat} ${styles.jwStatHighlight}`}
                      >
                        <i className="ti ti-trending-down" />
                        <span className={styles.jwStatLabel}>Saved</span>
                        <span
                          className={`${styles.jwStatValue} ${styles.jwStatValueGood}`}
                        >
                      ...
                      <div className={styles.jwStat}>
                        ...
                        <span className={styles.jwStatValue}>
                          {result.stats.originalLines} →{" "}
                          {result.stats.minifiedLines}
                        </span>
                      </div>
                      <button
                        type="button"
                        className={styles.jwDownloadBtn}
                        onClick={handleDownload}
                        aria-label="Download minified JavaScript"
                      >
                    ...
                    <div className={styles.jwMobActions}>
                      <button
                        type="button"
                        className={`${styles.jwMobBtn} ${
                          copiedKey === "mob" ? styles.copied : ""
                        }`}
                        onClick={() => handleCopy(result.output, "mob")}
                      >
                        <i
                          className={`ti ${
                            copiedKey === "mob" ? "ti-check" : "ti-copy"
                          }`}
                        />
        ...
        {tabView === "analysis" &&
          (result ? (
            ...
            <div className={styles.jwTabEmpty}>
              ...
              <h3>Code Analysis</h3>
              <p>
                Add JavaScript code in the Minify tab to see detailed analysis,
                code quality metrics, and linting results.
              </p>
              <button
                type="button"
          ...
          <div className={styles.jwHistory}>
            {history.length === 0 ? (
              <div className={styles.jwTabEmpty}>
                ...
                <p>
                  Your minification history will appear here after you download
                  a result.
                </p>
              </div>
            ) : (
              <>
                <div className={styles.jwHistoryHeader}>
                  <div className={styles.jwHistoryTitle}>
                    <i className="ti ti-history" />
                    History
                    <span className={styles.jwHistoryCount}>
                      {history.length}
                    </span>
                  </div>
                  <button
                    type="button"
                    className={styles.jwGhostBtn}
                ...
                <div className={styles.jwHistoryList}>
                  {history.map((entry) => (
                    <div
                      key={entry.id}
                      className={styles.jwHistoryItem}
                      onClick={() => {
                    ...
                    >
                      <div className={styles.jwHistoryItemLeft}>
                        ...
                        <div>
                          <div className={styles.jwHistoryItemTitle}>
                            {entry.title}
                          </div>
                      ...
                      <div className={styles.jwHistoryItemRight}>
                        <span className={styles.jwHistoryMode}>
                          {entry.options.mode}
                        </span>
      ...
      <div className={styles.jwFooter}>
        ...
        {result && (
          <div className={styles.jwFooterRight}>
            <span>{result.stats.functions} fn</span>
            <span>·</span>
            <span>{result.stats.variables} vars</span>
            <span>·</span>
            <span
              className={
                result.analysis.syntaxValid ? styles.jwValid : styles.jwInvalid
              }
            >
              <i
                className={`ti ${
                  result.analysis.syntaxValid ? "ti-check" : "ti-x"
                }`}
              />
