  ...
  const analysis = useMemo(() => {
    ...
    const securityAnalysis = {
      ...
      recommended: results.filter((r) => {
        const algo = HASH_ALGORITHMS[r.algorithm];
        return algo?.isSecure && !algo.isDeprecated && algo.bitLength >= 256;
      }).length,
    };
    ...
    const bitLengthDistribution = results.reduce(
      (acc, result) => {
        const bitLength = HASH_ALGORITHMS[result.algorithm]?.bitLength || 0;
          ...
          {analysis.securityAnalysis.recommended === 0 && (
            <div className={`${styles.haRecommendation} ${styles.haRecommendationInfo}`}>
              ...
              <div>
                ...
                SHA-512, or SHA-3 algorithms with at least 256-bit output.
              </div>
            </div>
          )}

          {analysis.fastestHash.executionTime > 100 && inputSize > 1024 * 100 && (
            <div className={`${styles.haRecommendation} ${styles.haRecommendationPerformance}`}>
              ...
              <div>
                ...
                using BLAKE2b or BLAKE2s for high-speed hashing.
              </div>
            </div>
          )}
          ...
          {inputSize > 1024 * 1024 && (
            <div className={`${styles.haRecommendation} ${styles.haRecommendationInfo}`}>
              ...
              <div>
                ...
                computation for better memory efficiency.
              </div>
            </div>
          )}
        </div>
