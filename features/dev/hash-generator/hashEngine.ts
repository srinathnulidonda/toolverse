...
export const HASH_ALGORITHMS: Record<HashAlgorithm, {
    label: string;
    description: string;
    bitLength: number;
...
export function verifyHash(
    ...
    return generateHash(input, options).then(result => 
...
export function compareHashes(hash1: string, hash2: string): {
    ...
    const similarity = Math.round(((Math.max(hash1.length, hash2.length) - differences) / Math.max(hash1.length, hash2.length)) * 100);
...
function generateHashMetadata(algorithm: HashAlgorithm, hash: string, format: HashFormat): HashMetadata {
...
export function calculateHashRate(totalHashes: number, timeMs: number): {
