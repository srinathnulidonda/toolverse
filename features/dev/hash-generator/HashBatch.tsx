...
export default function HashBatch({ algorithms, options, onComplete }: HashBatchProps) {
  ...
  const processAll = useCallback(async () => {
    if (items.length === 0 || isProcessing || algorithms.length === 0) return;
    ...
    for (let i = 0; i < updatedItems.length; i++) {
      ...
      try {
        ...
        setItems((prev) =>
          prev.map((p) =>
            p.id === item.id
              ? { ...p, status: "completed" as const, results, processingTime }
              : p
          )
        );
      } catch (error) {
        ...
        <div className={styles.hbUploadArea}>
          ...
          <div className={styles.hbTextInput}>
            <textarea
              ...
              disabled={isProcessing}
              aria-label="Text input for batch processing"
            />
        ...
        {items.length > 0 && (
          <div className={styles.hbBatchSummary}>
            ...
            <button
              ...
              onClick={processAll}
              disabled={isProcessing || items.length === 0 || algorithms.length === 0}
              aria-label={isProcessing ? `Processing ${progress}%` : "Process all items"}
            >
              ...
              <div key={item.id} className={styles.hbResultItem}>
                <div className={styles.hbResultHeader}>
                  ...
                  <div className={styles.hbResultActions}>
                    <div
                      className={`${styles.hbResultStatus} ${
                        styles[
                          `hbResultStatus${
                    ...
                    >
                      ...
                      {item.status === "processing" && (
                        <i className={`ti ti-loader ${styles.hbSpin}`} />
                      )}
                    ...
                    {item.status === "completed" && (
                      <button
                        type="button"
                        className={`${styles.hbCopyBtn}${
                          copiedId === item.id ? ` ${styles.copied}` : ""
                        }`}
                        onClick={() => copyResults(item)}
                        title="Copy all hashes"
                        aria-label="Copy all hashes for this item"
                      >
