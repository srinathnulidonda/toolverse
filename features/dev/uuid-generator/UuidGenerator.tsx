...
export default function UuidGenerator({
  ...
  const namespaceValidation = useMemo(() => {
    if (!needsNamespace || !useCustomNamespace || !customNamespace.trim()) return null;
    return validateUuid(customNamespace.trim());
  }, [needsNamespace, useCustomNamespace, customNamespace]);

  const namespaceInvalid = !!namespaceValidation && !namespaceValidation.valid;

  const canGenerate =
    !needsNamespace || (!!activeNamespace.trim() && !!name.trim() && !namespaceInvalid);
  ...
  const handleGenerate = useCallback(
    async (commit: boolean = false) => {
      ...
      try {
        ...
        if (needsNamespace) {
          options.namespace = activeNamespace.trim();
          options.name = name;
        }
    ...
    [version, format, uuidCase, activeNamespace, name, needsNamespace, canGenerate, onGenerated]
  );
  ...
  useEffect(() => {
    ...
    const delay = needsNamespace ? DEBOUNCE_MS : 0;
  ...
  }, [version, format, uuidCase, needsNamespace, canGenerate, activeNamespace, name]);

  return (
    <div className={styles.ugGenRoot}>
      <div className={styles.ugGenBody}>
        <div className={styles.ugGenCol}>
          {needsNamespace ? (
              ...
              {useCustomNamespace ? (
                <div className={styles.ugGenInputGroup}>
                  ...
                  <input
                    id="custom-namespace"
                    type="text"
                    className={`${styles.ugGenInput} ${namespaceInvalid ? styles.invalid : ""}`}
                  ...
                  />
                  {namespaceInvalid ? (
                    <span className={styles.ugGenInputError}>
                      {namespaceValidation?.error || "Invalid namespace UUID"}
                    </span>
                  ) : (
                    <span className={styles.ugGenInputHint}>Must be a valid UUID</span>
                  )}
                </div>
              ) : (
                <div className={styles.ugGenInputGroup}>
            ...
            <div className={styles.ugGenNamespace}>
              ...
              <p className={styles.ugGenInputHint}>
                {VERSION_INFO[version].label} does not require any additional configuration. Use
                the refresh button to generate a new value.
              </p>
            </div>
          )}
        ...
        <div className={styles.ugGenGutter}>
          <div className={styles.ugGenGutterLine} />
          <div className={styles.ugGenGutterNode}>
            <i className="ti ti-arrow-right" />
          </div>
          <div className={styles.ugGenGutterLine} />
        </div>

        <div className={`${styles.ugGenCol} ${styles.ugGenColResult}`}>
            ...
            <div className={styles.ugGenUuidBox}>
              {currentUuid ? (
                ...
                <span className={styles.ugGenPlaceholder}>
                  {needsNamespace ? "Enter namespace and name to generate" : "Click generate"}
                </span>
              )}
            </div>
