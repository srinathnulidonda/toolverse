// features/dev/html-formatter/ts/htmlEngine.ts
...
export function extractMetadata(html: string): HTMLMetadata {
  ...
  const hasSemanticHTML = semanticElements.some(
    (elem) => html.toLowerCase().includes(`<${elem}>`) || html.toLowerCase().includes(`<${elem} `)
  );
  ...
  validation.warnings.forEach((warning) => {
    if (warning.type === "accessibility") {
      accessibilityScore -=
        warning.severity === "high" ? 10 : warning.severity === "medium" ? 5 : 2;
    }
  });
