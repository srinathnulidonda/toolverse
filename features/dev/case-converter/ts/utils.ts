// features/dev/case-converter/ts/utils.ts
...
export function convertCase(
  ...
  switch (caseType) {
    case "camel":
      return words.map((w, i) => processWord(w, i > 0)).join("");

    case "pascal":
      return words.map((w) => processWord(w, true)).join("");
    ...
    case "param":
      return words.map((w) => processWord(w, true)).join("_");
